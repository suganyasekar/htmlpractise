const text = [
 ...
 const txtval = document.getElementById("para");
 const paratxt = document.getElementById("result");
 

  btn.addEventListener("click",function(e){
    e.preventDefault();
    let value = parseInt(txtval.value); 
    let random = Math.floor(Math.random() *text.length);
    console.log(random);
  
    if(isNaN(value)||value<=0||value>9)
    {
        paratxt.innerHTML = `<p>${text[random]}</p>`;
    }
    else{
